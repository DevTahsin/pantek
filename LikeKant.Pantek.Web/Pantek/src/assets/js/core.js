...
// 	targetX: jQuery(window).width() / 2,
// 	targetY: jQuery(window).height() / 2,
// 	currentX: jQuery(window).width() / 2,
// 	currentY: jQuery(window).height() / 2,
// 	easing: 0.2,
// 	init: function () {
// 		let $this_el = this.$el;
// 		// Cursor Move
...
// 			this.currentAnim = gsap.fromTo(current_slide, {
// 				scale: scale_from,
// 				onStart: function () {
...
ashade.progress = {
	init: function (this_el) {
		...
		jQuery(window).on('resize', this.layout(this_el));
	},
...
ashade.sScroll = {
	...
	animate: function () {
		...
		$('.ashade-content-scroll').css('transform', 'translate3d(0, -' + ashade.sScroll.current + 'px, 0)');
		requestAnimationFrame(ashade.sScroll.animate);
	},
	layout: function () {
		if ($('.ashade-content-scroll').length) {
			let this_content = $('.ashade-content-scroll').children('.ashade-content');
			this_content.css('min-height', '0px');

			// Set Body Height (for smooth scroll)
			if ($('.ashade-content-scroll').height() <= jQuery(window).height()) {
				let min_height = jQuery(window).height() - jQuery('footer#ashade-footer').height();

				if (!jQuery('body').hasClass('no-header-padding'))
					min_height = min_height - $('.ashade-content-scroll').children('.ashade-header-holder').height();

				this_content.css('min-height', min_height + 'px');
				$('.ashade-content-scroll').addClass('is-centered');
			} else {
				$('.ashade-content-scroll').removeClass('is-centered');
			}
			if (jQuery('body').hasClass('ashade-smooth-scroll')) {
				jQuery('body').height($('.ashade-content-scroll').height());
			}
		}
	}
};
...
ashade.init = function () {
	...
	jQuery('body').addClass('is-init');
	ashade.old_scroll_top = jQuery(window).scrollTop();
	...
	let ashade_mobile_header, mobile_menu_button, mobile_menu, mobile_menu_close;
	if (jQuery('nav.ashade-mobile-menu').length) {
		ashade_mobile_header = jQuery('<div class="ashade-mobile-header">');
		mobile_menu_button = jQuery('a.ashade-mobile-menu-button');
		mobile_menu = jQuery('nav.ashade-mobile-menu');
		mobile_menu_close = jQuery('a.ashade-mobile-menu-close')
	} else {
		ashade_mobile_header = jQuery('<div class="ashade-mobile-header">');
		mobile_menu_button = jQuery('<a href="#" class="ashade-mobile-menu-button"><i class="la la-bars"></i></a>').appendTo(ashade_mobile_header);
		mobile_menu = jQuery('<nav class="ashade-mobile-menu"></nav>').appendTo(jQuery('body'));
	...
	if (jQuery('body').hasClass('ashade-maintenance-wrap')) {
		...
		jQuery(document).on('click', '.ashade-contacts-toggler', function () {
			jQuery('body').addClass('contacts-shown');
		});
		jQuery(document).on('click', '.ashade-contacts-close', function () {
			jQuery('body').removeClass('contacts-shown');
		});
	}

	jQuery('header#ashade-header').find('.ashade-nav-block').append(ashade_mobile_header);

	if (jQuery('header#ashade-header').find('.ashade-nav').length) {
		if (!mobile_menu.find('.ashade-mobile-menu-inner').length) {
			mobile_menu.append('\
			<div class="ashade-mobile-menu-inner">\
				<div class="ashade-mobile-menu-content">\
					'+ jQuery('header#ashade-header').find('.ashade-nav').html() + '\
				</div>\
			</div>\
		');
	...
	mobile_menu_button.on('click', function () {
		jQuery('body').addClass('ashade-mobile-menu-shown').addClass('is-locked');
		ashade.old_scroll_top = jQuery(window).scrollTop();
	...
	mobile_menu_close.on('click', function () {
		...
		gsap.fromTo('.ashade-mobile-menu ul.main-menu > li',
			...
			},
			{
				...
				stagger: 0.1,
				onComplete: function () {
					jQuery('body').removeClass('ashade-mobile-menu-shown').removeClass('is-locked');
				}
			},
		);
	});
	...
	jQuery(document).on('click', 'a.ashade-aside-toggler', function (e) {
		e.preventDefault();
		jQuery('body').addClass('ashade-aside-shown').removeClass('ashade-menu-fade');
		ashade.old_scroll_top = jQuery(window).scrollTop();
	});
	jQuery('a.ashade-aside-close').on('click', function (e) {
		e.preventDefault();
		jQuery('body').removeClass('ashade-aside-shown');
	});
	...
	jQuery('nav.ashade-nav a').on('mouseenter', function () {
		jQuery('body').addClass('ashade-menu-fade');
	});
	jQuery('nav.ashade-nav').on('mouseleave', function () {
		jQuery('body').removeClass('ashade-menu-fade');
	});
	...
	jQuery('.ashade-back').on('click', function (e) {
		...
		if ($this.hasClass('is-message')) {
			jQuery('body').addClass('is-locked in-message-mode');
		...
		if ($this.hasClass('is-message-close')) {
			jQuery('body').addClass('is-locked').removeClass('in-message-mode');
			$this.parent().removeClass('is-loaded');
			gsap.to('#ashade-contacts-wrap', {
				opacity: 0,
				y: 150,
		...
		if ($this.hasClass('is-home-return')) {
			...
			gsap.fromTo('.ashade-content', 1, {
			...
			},
				{
					...
					duration: 1,
					onComplete: function () {
						if ($('.ashade-content-scroll').find('#ashade-home-works').length) {
							var $current_content = jQuery('#ashade-home-works');
						}
						if ($('.ashade-content-scroll').find('#ashade-home-contacts').length) {
			...
			gsap.to('.ashade-page-background', {
				...
				delay: 1,
				onComplete: function () {
					jQuery('body').removeClass('ashade-content-shown');
					jQuery('body').removeClass('is-locked');
				}
			});
		}
	...
	// Home Template
	if (jQuery('body').hasClass('ashade-home-template')) {
		// Home Links Events
		jQuery('.ashade-home-link').on('mouseenter', function () {
			jQuery('body').addClass('is-faded');
		}).on('mouseleave', function () {
			jQuery('body').removeClass('is-faded');
		}).on('click', function () {
			var $this = jQuery(this);
			ashade.cursor.$el.removeClass('int-link');
			jQuery('body').removeClass('is-faded').addClass('ashade-content-shown');
			...
			if (jQuery('body').hasClass('ashade-smooth-scroll')) {
				// jQuery('.ashade-content-scroll') = jQuery('body').find('.ashade-content-scroll');
				jQuery('body').height(jQuery('.ashade-content-scroll').height());
			}
			ashade.layout();
	...
	if (jQuery('.ashade-kenburns-slider').length) {
		ashade.kenburns = {
			...
			init: function () {
				...
				this_f.$el.find('.ashade-kenburns-slide').each(function () {
					let offset_x =50, //Math.random()*100
						offset_y =50; //Math.random()*100
	...
	if (jQuery('.ashade-lightbox-link').length) {
		...
		jQuery(document).on('click', '.ashade-lightbox-link', function (e) {
			...
			ashade.pswp.lightbox = new PhotoSwipe(jQuery('body').find('.pswp')[0], PhotoSwipeUI_Default, ashade.pswp.gallery[this_gallery], this_options);
...
ashade.layout = function () {
	...
	if (jQuery(window).width() > 760) {
		jQuery('body').removeClass('ashade-mobile-menu-shown');
	}

	// Header Space Holder
	if (typeof $ashade_header_holder !== 'undefined') {
		$ashade_header_holder.height(jQuery('header#ashade-header').height());
	}

	// Header Padding to Home Template
	if (jQuery('#ashade-home-works').length) {
		jQuery('#ashade-home-works').css('padding-top', jQuery('header#ashade-header').height() + 'px');
	}
	if (jQuery('#ashade-home-contacts').length) {
		jQuery('#ashade-home-contacts').css('padding-top', jQuery('header#ashade-header').height() + 'px');
	}
	...
	if (jQuery('.ashade-row-fullheight').length) {
		jQuery('.ashade-row-fullheight').each(function () {
			...
			if ($this.hasClass('exclude-header')) {
				minHeight = minHeight - jQuery('header#ashade-header').height();
			}
			if ($this.hasClass('exclude-footer')) {
				minHeight = minHeight - jQuery('footer#ashade-footer').height();
			}
	...
	jQuery('header#ashade-header').find('.ashade-menu-offset').removeClass('ashade-menu-offset');

	jQuery('header#ashade-header').find('.sub-menu').each(function () {
		var $this = jQuery(this),
			this_left = $this.offset().left,
			this_left_full = $this.offset().left + $this.width() + parseInt($this.css('padding-left'), 10) + parseInt($this.css('padding-right'), 10);

		if (this_left_full > jQuery(window).width()) {
...
ashade.loading = function () {
	...
	if (jQuery('.ashade-content').length) {
		...
		if (ashade.contentAnim) {
			ashade.contentAnim.kill();
		}
		ashade.contentAnim = gsap.from('.ashade-content', {
			opacity: 0,
			y: 100,
			duration: 1,
			delay: contentDelay,
	...
	setTimeout("jQuery('body').addClass('is-loaded')", 1500);
}

ashade.change_location = function (this_href) {
	ashade.cursor.$el.addClass('is-unloading');
	jQuery('body').addClass('is-locked');
	if (jQuery(window).width() < 760 && jQuery('body').hasClass('ashade-mobile-menu-shown')) {
		let setDelay = 0;
		jQuery('body').addClass('is-locked');
	...
	if (jQuery('body').hasClass('ashade-home-template')) {
		if (!jQuery('body').hasClass('ashade-home-state--contacts') && !jQuery('body').hasClass('ashade-home-state--works')) {
			var links_delay = 0.5,
				links_timeout = 0;
		} else {
			var links_delay = 1.1,
